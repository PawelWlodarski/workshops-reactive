...
class StreamsIntroExercise3GraphDSLSpec extends TestKit(ActorSystem("streams")) with FlatSpecLike
  ...
  "Source created from graph" should "save messages in external collection" in {
    ...
    val sourceWithHistory = GraphDSL.create() { implicit builder =>
      ...
      // source sends to broadcast
      //one of broadcast outputs to historySink

      SourceShape(???)  //return other broadcast output
    }
  ...
  "sink create from graph" should "zip inputs" in {
    ...
    val zippingSink = GraphDSL.create(historySink) { implicit  builder => importedSink =>
      ...
      //add zip [String,Int] to builder

      //source to zipper
      //zipper to imported sink
