...
class SlickQueriesAnswers extends FunSuite with MustMatchers with DatabaseOperations {
  ...
  test("select 2 most expensive products") {
    ...
    withProductsDB { db =>
      ...
      val query = ProductsDao.dao
        ...
        .take(2)
        .map(_.mapTo[ProductIdProjection])
...
object ProductsDao {
  ...
  lazy val dao = new TableQuery(new ProductsDao(_)) {
    ...
    def add(name: String, price: Double): DBIO[Int] =
      this.returning(this.map(_.id)) += (0, name, price)

    def getMaxId: DBIO[Option[Int]] = this.map(_.id).max.result

    def addIfNotExist(name: String, price: Double): DBIO[Int] = {
      val data = Query(name, price)
      val exists: Rep[Boolean] = this.filter(_.name === name).exists
      val select = data.filterNot(_ => exists)
      this.map(r => (r.name, r.price)).forceInsertQuery(select)
    }
